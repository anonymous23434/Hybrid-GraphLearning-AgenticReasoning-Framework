...
def plot_fraud_distribution(df):
    ...
    if 'is_fraudulent' in df.columns:
        ...
        plt.ylabel("Count")
        plt.tight_layout()
        plt.savefig("plots/fraud_distribution.png", dpi=300)
        plt.close()
        print("Fraud distribution plot saved as 'plots/fraud_distribution.png'\n")
    else:
...
def correlation_analysis(df, numeric_df):
    ...
    if 'is_fraudulent' in df.columns:
        ...
        plt.tight_layout()
        plt.savefig("plots/correlation_with_fraud.png", dpi=300)
        plt.close()
        print("Correlation plot saved as 'plots/correlation_with_fraud.png'\n")
        return top_corr
    return None
...
def plot_distributions(numeric_df):
    ...
    for col in cols_to_plot:
        ...
        plt.tight_layout()
        plt.savefig(f"plots/hist_{col}.png", dpi=300)
        plt.close()
        print(f"Histogram for {col} saved as 'plots/hist_{col}.png'")

# -----------------------------
# 8. Outlier Detection via Boxplots
# -----------------------------
def plot_boxplots(numeric_df):
    """Plot and save boxplots for the first 5 numeric features."""
    cols_to_plot = numeric_df.columns[:5]  # First 5 numeric features
    for col in cols_to_plot:
        plt.figure(figsize=(6, 4))
        ...
        plt.xlabel(col)
        plt.tight_layout()
        plt.savefig(f"plots/boxplot_{col}.png", dpi=300)
        plt.close()
        print(f"Boxplot for {col} saved as 'plots/boxplot_{col}.png'")
...
def plot_pairplot(df, top_corr):
    ...
    if 'is_fraudulent' in df.columns and top_corr is not None:
        ...
        plt.savefig("plots/pairplot_top_features.png", dpi=300)
        plt.close()
        print("Pairplot saved as 'plots/pairplot_top_features.png'\n")
...
{'Fraud distribution plot saved at `plots/fraud_distribution.png`' if 'is_fraudulent' in df.columns else 'Column `is_fraudulent` not found'}

## 5. Correlation with Fraud
{top_corr_table}

## 6. Distributions and Outliers
- Histograms saved at `plots/hist_<column>.png`
- Boxplots saved at `plots/boxplot_<column>.png`

## 7. Pairplot
{'Pairplot saved at `plots/pairplot_top_features.png`' if top_corr is not None else 'Column `is_fraudulent` not found'}
...
{skew_bottom_table}
"""
    with open("plots/analysis_report.md", "w") as f:
        f.write(report)
    print("Summary report saved as 'plots/analysis_report.md'\n")
    print("Analysis complete! Check the 'plots' directory for visualizations and report.\n")
